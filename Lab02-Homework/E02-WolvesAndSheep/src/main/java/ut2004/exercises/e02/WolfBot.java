...
import cz.cuni.amis.pogamut.base.utils.math.DistanceUtils.IDistanceFilter;
...
public class WolfBot extends UT2004BotModuleController {
    ...
    public static Player[] targets = new Player[] {null, null, null};
    /**
    ...
    public void logic() throws PogamutException {
    	...
    	{
    		move.moveTo(targets[instance]);
    		if (this.bot.getLocation().getDistance(targets[instance].getLocation()) > 150)
    			move.dodgeTo(targets[instance], true);
    		else
    			move.dodgeTo(targets[instance], false);
    	}	
    }

    private void locateTarget()
    {
    	int otherWolf = isAlpha() ? 2 : 1;

    	if (canSeeSheep())
    	{
    		targets[instance] = getNearestSheep();
    		if (targets[otherWolf] == null)
    			return;
    		
    		if (targets[otherWolf].getId().equals(targets[instance].getId()))
    		{
    			targets[instance] = getSecondNearestSheep();
    			
    			if (targets[instance].getName().equals(targets[otherWolf].getName()) && sheeps > 2)
    ...
    private boolean canSeeSheep()
    {
    	for (Player player : players.getVisiblePlayers().values()) {
			if (Utils.isSheep(player)) return true;
    	}
    	
    	return false;	
    }
    
    private Player getNearestSheep()
    {
    	return DistanceUtils.getNearest(players.getVisiblePlayers().values(),
    			info.getLocation(),
    			new IDistanceFilter<Player>() {

					@Override
					public boolean isAccepted(Player object, ILocated target, double distanceToTarget) {
						return Utils.isSheep(object);
					}
    		
    	});
    }
    	 private Player getSecondNearestSheep()
    	    {
    	    	return DistanceUtils.getNthNearest(1, players.getVisiblePlayers().values(),
    	    			info.getLocation(),
    	    			new IDistanceFilter<Player>() {
