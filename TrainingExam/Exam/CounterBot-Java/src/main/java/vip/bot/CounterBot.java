...
public class CounterBot extends UT2004BotVIPController<UT2004Bot> {
    ...
    public void logic() {
    	...
    	log.info("BACK: " + players.getPlayerName(backBuddy, true));
    	log.info("FRONT: " + players.getPlayerName(frontBuddy, true));
    	...
    	if (backBuddy == info.getId()) {
    		if (engageInCombat())
    			return;
    		/*if (isVIPProtected() && !buddyNeedsHelp()) {
    	...
    	if (frontBuddy == info.getId()) {
    		...
    		/*if (this.isVIPProtected() && !buddyNeedsHelp()) {
    			targetItem = getTargetItem();
    			navigateToItem();
    			return;
    		}*/
    ...
    private void assignBuddy() {   	  	    		
    	...
    	if (sortedAllies.size() >= 2) {
			if (info.getId() == sortedAllies.get(0)) {
				while (sortedAllies.get(front) == sortedAllies.get(back) && sortedAllies.size() > back + 1)
					back++;
				backBuddy = sortedAllies.get(back);
				frontBuddy = sortedAllies.get(front);
    ...
    private void frontBuddyMovement() {    	
    	...
    	if (vipRunningTo == null) {
    		move.moveTo(vipLocation.add(info.getVelocity()));
    		return;
    	}    	
    	Location frontBuddyLocation = vipLocation.add(vipRunningTo.sub(vipLocation).getNormalized().scale(150));
    	
    	
    	if (getPathDistance(frontBuddyLocation, info.getLocation()) > 200)
